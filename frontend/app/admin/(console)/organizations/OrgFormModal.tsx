...
/// Create a tenant organization. The slug is generated from the name by the
/// backend (suffixed automatically if taken), so the form is just a name.
export function OrgFormModal({
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      onSaved(`Created organization ${org.name}.`);
    } catch (err) {
  ...
  return (
    <ModalShell onClose={onClose} maxWidth={520}>
      <ModalHeader
        ...
        subtitle="A tenant organization users can be added to"
        onClose={onClose}
      />
